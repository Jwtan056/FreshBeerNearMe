...
<body>
	<div id="main">
		<?php
		//User Creation
		if (isset($_POST['CreateUserAccount'])) {
			$NewUserAccount = new User($_POST['_id']);
			$NewUserAccount->setfirstName($_POST['FirstName']);
			$NewUserAccount->setlastName($_POST['LastName']);
			$NewUserAccount->setGender($_POST['Gender']);
			$NewUserAccount->setEmail($_POST['Email']);
			$NewUserAccount->setContact($_POST['contact']);
			$NewUserAccount->setDob($_POST['dob']);
			$NewUserAccount->setPassword($_POST['Password']);

			
			if ($NewUserAccount->CreateUserAccount()) {
		...
		<div id="register" class="container">
			<form class="form-horizontal" role="form" action='CreateUserAccount.php' method='POST'>
				<h2>Registration</h2>
				...
				<p><input type='submit' value='Create account' name='CreateUserAccount'></p>
			</form>
		</div>
	</div>
</body>
